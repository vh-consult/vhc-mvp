...
export async function fetchDoctorClients(id: string) {
  try {
    ...
    const doctor = await Doctor.findOne({ id }).populate({
      path: "clients",
      select: "firstName lastName gender email photo dateOfBirth",
    });
    if (!doctor) throw new Error("Doctor not found");
...
export async function fetchOngoing(doctorId: string) {
  try {
    await connectToDatabase();
    const doctor = await Doctor.findOne({ id: doctorId }).populate({
      path: "ongoingSession",
...
export async function fetchUpcoming(doctorId: string) {
  try {
    ...
    const doctor = await Doctor.findOne({ id: doctorId }).populate({
      path: "appointments",
      populate: [
        { path: "patient", select: "firstName lastName healthRecord" },
      ],
