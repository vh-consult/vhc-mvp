...
export async function editConsultationSummary(
  ...
  try {
    ...
    if (!doctor.consultationHistory.includes(sessionId))
      throw new Error("You can't edit this session");
...
export async function fetchAllConsultationSummary() {
  try {
    ...
    if (sessions.countDocuments.length > 0)
      throw new Error("No consultation sessions yet");
...
export async function fetchConsultationSession(consultationId: string) {
  try {
    ...
    const session = await Consultation.findById(consultationId).populate([
      {
        path: "patient",
        select: "firstName lastName healthRecord dateOfBirth email",
      },
