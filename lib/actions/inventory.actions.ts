...
export async function addToInventory(adminId: string, drugData: DrugParams) {
  try {
    await connectToDatabase();
    const admin = await PharmacyAdmin.findById({ id: adminId }).populate(
      "company"
    );
    ...
    try {
      ...
      if (shop.companyType !== "Pharmacy")
        throw new Error("Company is not a pharmacy");
...
export async function removeFromInventory(
  ...
  try {
    ...
    if (!shop.inventory.includes(drugId))
...
export async function getDrug(drugId: string, shopId: string) {
  try {
    ...
    const drugInInventory = inventory.some(
      (item: any) => item._id.toString() === drugId
    );
