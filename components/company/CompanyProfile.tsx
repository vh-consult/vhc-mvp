...
const CompanyProfile = ({ companyId }: { companyId: string }) => {
    ...
    const navsForRendering = useMemo(() => ({
        Hospital: [
            {
                label: 'About',
                componentToRender: <AboutComponent companyDescription={companyData?.description!} />
    ...
    }), [companyData]); // Memoize to prevent unnecessary re-creation
    ...
    useEffect(() => {
        ...
        request();
    }, [companyId, navsForRendering]); // Now, navsForRendering is stable
            ...
            <div className="w-full h-[25%] bg-white relative px-8 py-3">
                ...
                <div className="w-[40%] flex flex-between mt-3">
                    {companyData?.companyType &&
                        navsForRendering[companyData.companyType]?.map((component, index) => (
                            <span
                                key={index}
                                onClick={() => setActive(component)}
                                className={`
